...
namespace BlazorWebRTC {
    export class PeerConnection {
        ...
        public async init(helper, conf) {
            this.helper = helper;
            this.localConnection = new RTCPeerConnection(conf);
            this.localConnection.onicecandidate = async (evt) => {
                await helper.invokeMethodAsync("_onicecandidate", evt);
            };
            this.localConnection.onicecandidateerror = async (evt) => {
                await helper.invokeMethodAsync("_onicecandidateerror", evt);
            };
            ...
            this.localConnection.onconnectionstatechange = async (evt) => {
                await helper.invokeMethodAsync('_onconnectionstatechange', evt);
            };
            this.localConnection.onsignalingstatechange = async (evt) => {
                await helper.invokeMethodAsync('_onsignalingstatechange', evt);
            };
        }
...
export function create(dotnetHelper, conf) {
    connection = new BlazorWebRTC.PeerConnection();
    connection.init(dotnetHelper, conf);
    return connection;
}
